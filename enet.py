...
def generate_model(train_mode=True, pretrained_weights=cfg.pretrained_weights):

    model = ENet(cfg.n_classes, train_mode)
...
class SpatialSoftmax(torch.nn.Module):
    def __init__(self, temperature=1):
        ...
        if temperature:
            self.temperature = Parameter(torch.ones(1)*temperature).to(cfg.device)
        else:
            self.temperature = 1.

    def forward(self, feature):
        feature = feature.view(-1, feature.shape[1] * feature.shape[2])
...
class ENet(nn.Module):
    ...
    """
    def __init__(self, num_classes, sad=False, encoder_relu=False, decoder_relu=True):
        super().__init__()
    ...
    def at_gen(self, x1, x2):
        ...
        if x1.size() != x2.size():
            ...
            x2 = torch.squeeze(self.at_gen_upsample(x2))
            x2 = sps(x2)
    ...
    def forward(self, x):
        ...
        if self.sad:
            return x, loss_1 + loss_2 + loss_3
        else:
            return x


if __name__ == '__main__':
    tensor = torch.ones((8, cfg.input_image_c, cfg.input_image_h, cfg.input_image_w)).to(cfg.device)
